...
from app.models import (User, MembershipApplication, NewsPost, Event,
                        Resource, FAQ, ContactMessage)
    ...
    with app.app_context():
        ...
        for name, email, mtype, org, city, state, status in apps_data:
            if not MembershipApplication.query.filter_by(email=email).first():
                app_obj = MembershipApplication(
                    full_name=name, email=email, membership_type=mtype,
                    organization=org, city=city, state=state, status=status,
        ...
        posts = [
            ('PAAI Advocacy Leads to Landmark Patient Rights Amendment',
             'policy-win-patient-rights-2024', 'Policy Win',
            ...
            ('Annual National Patient Advocacy Summit 2024 — Highlights',
             'advocacy-summit-2024', 'Community',
            ...
            ('PAAI Signs MoU with WHO India for Rare Disease Programme',
             'who-india-mou-2024', 'Partnership',
        ...
        for title, slug, cat, excerpt, content, published in posts:
            if not NewsPost.query.filter_by(slug=slug).first():
                post = NewsPost(
                    title=title, slug=slug, category=cat, excerpt=excerpt,
                    content=content, is_published=published,
        ...
        # ── Events ──
        events_data = [
            ('Understanding Your Rights: PMJAY & Insurance Navigation', 'Webinar', 'Online (Zoom)',
             datetime.utcnow() + timedelta(days=20), True),
            ('Rare Disease Awareness Day — National Symposium 2025', 'Conference', 'AIIMS Auditorium, New Delhi',
             datetime.utcnow() + timedelta(days=38), True),
            ('Caregiver Wellbeing Workshop — Mumbai Chapter', 'Workshop', 'Kokilaben Hospital, Mumbai',
             datetime.utcnow() + timedelta(days=55), False),
        ]
        for title, etype, loc, edate, is_free in events_data:
            if not Event.query.filter_by(title=title).first():
                ev = Event(title=title, event_type=etype, location=loc,
                           event_date=edate, is_free=is_free, is_published=True)
                db.session.add(ev)
        print('  ✓ Events seeded')

        # ── Resources ──
        resources_data = [
            ('Know Your Healthcare Rights in India', 'Patient Rights', 'PDF',
             'Comprehensive guide to patient rights under Indian law.'),
            ('Navigating Health Insurance Claims', 'Insurance', 'Toolkit',
             'Step-by-step toolkit for filing and appealing health insurance claims.'),
            ('Mental Wellness for Patients & Caregivers', 'Mental Health', 'PDF',
             'Evidence-based strategies for managing chronic illness.'),
            ('Rare Disease Advocacy Handbook', 'Rare Disease', 'PDF',
             'Practical guide for patients with rare diseases.'),
            ('Caregiver Burnout Prevention Guide', 'Caregiver', 'PDF',
             'Self-assessment tools and coping strategies for caregivers.'),
            ('Medical Negligence & Legal Recourse', 'Legal', 'PDF',
        ...
        for title, cat, rtype, desc in resources_data:
            if not Resource.query.filter_by(title=title).first():
                r = Resource(title=title, category=cat, resource_type=rtype,
        ...
        faqs_data = [
            ('How do I join PAAI?',
             ...
             'Membership', 'join,member,apply,registration'),
            ('What is the cost of membership?',
             ...
             'Membership', 'cost,fee,price,paid,free'),
            ('How can caregivers get support from PAAI?',
             ...
             'Programs', 'caregiver,support,caring,family'),
            ('Does PAAI help with insurance disputes?',
             ...
             'Resources', 'insurance,claim,dispute,PMJAY'),
        ]
        for q, a, cat, kw in faqs_data:
            if not FAQ.query.filter_by(question=q).first():
                faq = FAQ(question=q, answer=a, category=cat, keywords=kw)
                db.session.add(faq)
        print('  ✓ FAQs seeded')

        # ── Contact messages ──
        msgs_data = [
            ('Suresh Patel', 'suresh@gmail.com', 'Membership Query', 'I would like to know more about joining PAAI as an individual advocate.'),
            ('Nirmala Devi', 'nirmala.d@yahoo.com', 'Caregiver Support', 'I am caring for my mother with Parkinson\'s and need guidance.'),
        ]
